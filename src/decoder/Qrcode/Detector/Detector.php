<?php
    ...
    use Coco\qrcode\decoder\Common\DetectorResult;
    use Coco\qrcode\decoder\Common\GridSampler;
    use Coco\qrcode\decoder\Common\PerspectiveTransform;
    use Coco\qrcode\decoder\FormatException;
    use Coco\qrcode\decoder\NotFoundException;
    use Coco\qrcode\decoder\Qrcode\Decoder\Version;
...
class Detector
{
     ...
     *
     * @param array|null $hints optional $hints hints to detector
     *
     * @return DetectorResult <a href="psi_element://DetectorResult">DetectorResult</a> encapsulating results of detecting a QR Code
     * @throws NotFoundException if QR Code cannot be found
     */
    final public function detect(array $hints = null): DetectorResult
    {

        $resultPointCallback = ($hints !== null && array_key_exists('NEED_RESULT_POINT_CALLBACK', $hints)) ? $hints['NEED_RESULT_POINT_CALLBACK'] : null;
    ...
    final protected function processFinderPatternInfo(FinderPatternInfo $info): DetectorResult
    {
    ...
    {
        $moduleSizeEst1 = $this->sizeOfBlackWhiteBlackRunBothWays((int)$pattern->getX(), (int)$pattern->getY(), (int)$otherPattern->getX(), (int)$otherPattern->getY());
     ...
     */
    private static function computeDimension($topLeft, $topRight, $bottomLeft, float $moduleSize): int
    {
     ...
     */
    final protected function findAlignmentInRegion(float $overallEstModuleSize, int $estAlignmentX, int $estAlignmentY, float $allowanceFactor)
    {
    ...
    private static function createTransform($topLeft, $topRight, $bottomLeft, $alignmentPattern, int $dimension): PerspectiveTransform
    {
    ...
    private static function sampleGrid($image, PerspectiveTransform $transform, int $dimension): BitMatrix
    {
