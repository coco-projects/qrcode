<?php
    ...
    use ValueError;
    use Coco\qrcode\decoder\Common\BitSource;
    use Coco\qrcode\decoder\Common\CharacterSetECI;
    use Coco\qrcode\decoder\Common\DecoderResult;
...
final class DecodedBitStreamParser
{
     ...
     */
    public static function decode(array $bytes, Version $version, ErrorCorrectionLevel $ecLevel, array|null $hints): DecoderResult
    {
     ...
     * @psalm-param '' $result
     */
    private static function decodeHanziSegment(BitSource $bits, string &$result, int $count): void
    {
    ...
    private static function decodeNumericSegment(BitSource $bits, string &$result, int $count): void
    {
    ...
    private static function decodeAlphanumericSegment(BitSource $bits, string &$result, int $count, bool $fc1InEffect): void
    {
    ...
    private static function decodeByteSegment(BitSource $bits, string &$result, int $count, CharacterSetECI|null $currentCharacterSetECI, array &$byteSegments, array|null $hints): void
    {
        // Don't crash trying to read more bits than we have available.
    ...
    private static function decodeKanjiSegment(BitSource $bits, string &$result, int $count): void
    {
        // Don't crash trying to read more bits than we have available.
        if ($count * 13 > $bits->available()) {
            throw new FormatException("Trying to read more bits than we have available");
