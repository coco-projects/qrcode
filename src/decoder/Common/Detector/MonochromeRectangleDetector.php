<?php
    ...
    use Coco\qrcode\decoder\BinaryBitmap;
...
class MonochromeRectangleDetector
{
     ...
     * white, in an image.</p>
     *
     * @return ResultPoint <a href="psi_element://ResultPoint">ResultPoint</a>[] describing the corners of the
     *                     rectangular region. The first and last points are opposed on the diagonal, as are the
     *                     second and third. The first point will be the topmost point and the last, the
     *                     bottommost. The second point will be leftmost and the third, the rightmost
     ...
     *
     * @param int|float $centerX     center's x component (horizontal)
     * @param int|float $deltaX      same as deltaY but change in x per step instead
     * @param int       $left
     * @param int       $right
     * @param int|float $centerY     center's y component (vertical)
     * @param float|int $deltaY      change in y per step. If scanning up this is negative; down, positive;
     *                               left or right, 0
     * @param int       $top
     * @param int       $bottom
     * @param int|float $maxWhiteRun maximum run of white pixels that can still be considered to be within
     *                               the barcode
     *
     * @throws NotFoundException if such a point cannot be found
     */
    private function findCornerFromCenter(int|float $centerX, int|float $deltaX, int $left, int $right, int|float $centerY, float|int $deltaY, int $top, int $bottom, int|float $maxWhiteRun): ResultPoint
    {
        $lastRange = null;
     ...
     *
     * @return (float|int)[]|null with start and end of found range, or null if no such range is found (e.g. only
     *                            white was found)
     *
     */
