...
final class HybridBinarizer extends GlobalHistogramBinarizer
{
     ...
     */
    private static function calculateBlackPoints(array|BitMatrix $luminances, float $subWidth, float $subHeight, float $width, float $height): array
    {
     ...
     */
    private static function calculateThresholdForBlock(array|BitMatrix $luminances, float $subWidth, float $subHeight, float $width, float $height, array $blackPoints, BitMatrix $matrix): void
    {
        for ($y = 0; $y < $subHeight; $y++) {
            $yoffset    = ($y << self::$BLOCK_SIZE_POWER);
            $maxYOffset = $height - self::$BLOCK_SIZE;
     ...
     */
    private static function thresholdBlock(array|BitMatrix $luminances, int|float $xoffset, int|float $yoffset, int $threshold, float $stride, BitMatrix $matrix): void
    {
    ...
    public function createBinarizer($source): static
    {
